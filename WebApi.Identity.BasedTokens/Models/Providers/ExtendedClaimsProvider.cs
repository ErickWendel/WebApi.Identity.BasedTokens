using System;
using System.Collections.Generic;
using System.Linq;
using System.Security.Claims;
using System.Web;

namespace WebApi.Identity.BasedTokens.Models.Providers
{
    public static class ExtendedClaimsProvider
    {
        public static IEnumerable<Claim> GetClaims(ApplicationUser user)
        {

            List<Claim> claims = new List<Claim>();

            switch (user.PerfilSecundario)
            {
                case (int)PerfilSecundario.Provisionador:
                    claims.Add(CreateClaim("PerfilSecundario", PerfilSecundario.Provisionador.ToString()));
                    break;
                default:
                    claims.Add(CreateClaim("PerfilSecundario", "Qualquer um"));
                    break;
            }
            //var daysInWork = (DateTime.Now.Date - user.JoinDate).TotalDays;

            //if (daysInWork > 90)
            //{
            //    claims.Add(CreateClaim("FTE", "1"));

            //}
            //else
            //{
            //    claims.Add(CreateClaim("FTE", "0"));
            //}

            return claims;
        }

        public static Claim CreateClaim(string type, string value)
        {
            return new Claim(type, value, ClaimValueTypes.String);
        }

    }
}